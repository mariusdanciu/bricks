...
impl IntoIterator for DeviceExtensions {
    ...
    type IntoIter = std::array::IntoIter<Self::Item, 288usize>;
    #[inline]
    ...
    pub(super) fn check_requirements(
        ...
        if self.ext_video_encode_h264 {
            if !supported.ext_video_encode_h264 {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_video_encode_h264`, but this extension is not supported by the physical device" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_video_encode_h265 {
            if !supported.ext_video_encode_h265 {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_video_encode_h265`, but this extension is not supported by the physical device" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_ycbcr_2plane_444_formats {
        ...
        if self.qcom_render_pass_transform {
            ...
            if !(instance_extensions.khr_surface) {
                return Err(Box::new(crate::ValidationError {
                    problem: "contains `qcom_render_pass_transform`".into(),
                    requires_one_of: crate::RequiresOneOf(&[crate::RequiresAllOf(&[
                        crate::Requires::InstanceExtension("khr_surface"),
                    ])]),
                    ..Default::default()
