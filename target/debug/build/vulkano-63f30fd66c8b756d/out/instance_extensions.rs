...
pub struct InstanceExtensions {
    ...
    #[doc = "- [Vulkan documentation](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VK_EXT_layer_settings.html)"]
...
impl InstanceExtensions {
    ...
    pub const fn count(self) -> u64 {
        self.khr_android_surface as u64
            + self.khr_device_group_creation as u64
            + self.khr_display as u64
            + self.khr_external_fence_capabilities as u64
            ...
            + self.khr_portability_enumeration as u64
            + self.khr_surface as u64
            + self.khr_surface_protected_capabilities as u64
            + self.khr_wayland_surface as u64
            + self.khr_win32_surface as u64
            + self.khr_xcb_surface as u64
            + self.khr_xlib_surface as u64
            + self.ext_acquire_drm_display as u64
            + self.ext_acquire_xlib_display as u64
            + self.ext_debug_report as u64
            + self.ext_debug_utils as u64
            + self.ext_direct_mode_display as u64
            + self.ext_directfb_surface as u64
            + self.ext_display_surface_counter as u64
            + self.ext_headless_surface as u64
            + self.ext_layer_settings as u64
            + self.ext_metal_surface as u64
            + self.ext_surface_maintenance1 as u64
            + self.ext_swapchain_colorspace as u64
            + self.ext_validation_features as u64
            + self.ext_validation_flags as u64
            + self.fuchsia_imagepipe_surface as u64
            + self.ggp_stream_descriptor_surface as u64
            + self.google_surfaceless_query as u64
            + self.lunarg_direct_driver_loading as u64
            + self.mvk_ios_surface as u64
            + self.mvk_macos_surface as u64
            + self.nn_vi_surface as u64
...
impl InstanceExtensions {
    #[doc = r" Checks enabled extensions against the library support,"]
    #[doc = r" and checks for required API version."]
    pub(super) fn check_requirements(
        ...
        if self.khr_android_surface {
            if !supported.khr_android_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_android_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_device_group_creation {
            if !supported.khr_device_group_creation {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_device_group_creation`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_display {
            if !supported.khr_display {
                return Err(Box::new(crate::ValidationError {
                    problem:
                        "contains `khr_display`, but this extension is not supported by the library"
                            .into(),
                    ..Default::default()
                }));
            }
        ...
        if self.khr_external_fence_capabilities {
            if !supported.khr_external_fence_capabilities {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_external_fence_capabilities`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_external_memory_capabilities {
            if !supported.khr_external_memory_capabilities {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_external_memory_capabilities`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_external_semaphore_capabilities {
            if !supported.khr_external_semaphore_capabilities {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_external_semaphore_capabilities`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_get_display_properties2 {
            if !supported.khr_get_display_properties2 {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_get_display_properties2`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_get_physical_device_properties2 {
            if !supported.khr_get_physical_device_properties2 {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_get_physical_device_properties2`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_get_surface_capabilities2 {
            if !supported.khr_get_surface_capabilities2 {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_get_surface_capabilities2`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_portability_enumeration {
            if !supported.khr_portability_enumeration {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_portability_enumeration`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_surface {
            if !supported.khr_surface {
                return Err(Box::new(crate::ValidationError {
                    problem:
                        "contains `khr_surface`, but this extension is not supported by the library"
                            .into(),
                    ..Default::default()
                }));
            }
        ...
        if self.khr_surface_protected_capabilities {
            if !supported.khr_surface_protected_capabilities {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_surface_protected_capabilities`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        ...
        if self.khr_wayland_surface {
            if !supported.khr_wayland_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_wayland_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_win32_surface {
            if !supported.khr_win32_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_win32_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_xcb_surface {
            if !supported.khr_xcb_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_xcb_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.khr_xlib_surface {
            if !supported.khr_xlib_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `khr_xlib_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_acquire_drm_display {
            if !supported.ext_acquire_drm_display {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_acquire_drm_display`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_acquire_xlib_display {
            if !supported.ext_acquire_xlib_display {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_acquire_xlib_display`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_debug_report {
            if !supported.ext_debug_report {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_debug_report`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_debug_utils {
            if !supported.ext_debug_utils {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_debug_utils`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_direct_mode_display {
            if !supported.ext_direct_mode_display {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_direct_mode_display`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_directfb_surface {
            if !supported.ext_directfb_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_directfb_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_display_surface_counter {
            if !supported.ext_display_surface_counter {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_display_surface_counter`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_headless_surface {
            if !supported.ext_headless_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_headless_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_layer_settings {
            if !supported.ext_layer_settings {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_layer_settings`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_metal_surface {
            if !supported.ext_metal_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_metal_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_surface_maintenance1 {
            if !supported.ext_surface_maintenance1 {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_surface_maintenance1`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_swapchain_colorspace {
            if !supported.ext_swapchain_colorspace {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_swapchain_colorspace`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_validation_features {
            if !supported.ext_validation_features {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_validation_features`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ext_validation_flags {
            if !supported.ext_validation_flags {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ext_validation_flags`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.fuchsia_imagepipe_surface {
            if !supported.fuchsia_imagepipe_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `fuchsia_imagepipe_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.ggp_stream_descriptor_surface {
            if !supported.ggp_stream_descriptor_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `ggp_stream_descriptor_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.google_surfaceless_query {
            if !supported.google_surfaceless_query {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `google_surfaceless_query`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.lunarg_direct_driver_loading {
            if !supported.lunarg_direct_driver_loading {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `lunarg_direct_driver_loading`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.mvk_ios_surface {
            if !supported.mvk_ios_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `mvk_ios_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.mvk_macos_surface {
            if !supported.mvk_macos_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `mvk_macos_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.nn_vi_surface {
            if !supported.nn_vi_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `nn_vi_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.nv_external_memory_capabilities {
            if !supported.nv_external_memory_capabilities {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `nv_external_memory_capabilities`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        if self.qnx_screen_surface {
            if !supported.qnx_screen_surface {
                return Err (Box :: new (crate :: ValidationError { problem : "contains `qnx_screen_surface`, but this extension is not supported by the library" . into () , .. Default :: default () })) ;
            }
        }
        Ok(())
    }
    ...
    pub(super) fn enable_dependencies(
        ...
        if self.khr_external_fence_capabilities {
            if !(api_version >= crate::Version::V1_1 || self.khr_get_physical_device_properties2) {
                assert!(
                    ...
                    "The instance extension `{}` is enabled, and it requires \
                                        ...
                                        does not support the required extension. \
                                        This is a bug in the Vulkan driver.",
                    "khr_external_fence_capabilities", "khr_get_physical_device_properties2",
                );
            ...
            if !(api_version >= crate::Version::V1_1 || self.khr_get_physical_device_properties2) {
                assert!(
                    ...
                    "The instance extension `{}` is enabled, and it requires \
                                        ...
                                        does not support the required extension. \
                                        This is a bug in the Vulkan driver.",
                    "khr_external_memory_capabilities", "khr_get_physical_device_properties2",
                );
                self.khr_get_physical_device_properties2 = true;
            }
        }
            ...
            if !(api_version >= crate::Version::V1_1 || self.khr_get_physical_device_properties2) {
                assert!(
                    ...
                    "The instance extension `{}` is enabled, and it requires \
                                        ...
                                        does not support the required extension. \
                                        This is a bug in the Vulkan driver.",
                    "khr_external_semaphore_capabilities", "khr_get_physical_device_properties2",
                );
                self.khr_get_physical_device_properties2 = true;
            }
        }
            ...
            if !(self.khr_get_surface_capabilities2) {
                assert!(
                    ...
                    "The instance extension `{}` is enabled, and it requires \
                                        ...
                                        does not support the required extension. \
                                        This is a bug in the Vulkan driver.",
                    "khr_surface_protected_capabilities", "khr_get_surface_capabilities2",
                );
